...
const initialState = {
        ...
        userProfile: {
                gender: 'Female',
                weight: 143, // 65kg converted to lbs
                height: 65, // 165cm converted to inches
                age: 30,
        ...
        waterData: [],
        waterGoal: 64, // Default 64oz (8 cups) daily goal
...
const nutritionReducer = (state, action) => {
        switch (action.type) {
                ...
                case 'ADD_WEIGHT_ENTRY':
                        ...
                        return {
                                ...state,
                                weightData: [...state.weightData, newWeightEntry],
                        };
                ...
                case 'ADD_WATER_ENTRY':
                        ...
                        return {
                                ...state,
                                waterData: [...state.waterData, newWaterEntry],
                        };
...
export const NutritionProvider = ({ children }) => {
                                ...
                                try {
                                        ...
                                        if (userDoc.exists()) {
                                                ...
                                                const firestoreData = {
                                                        ...
                                                        weightGoal: userData.weightGoal || null,
                                                        waterData: userData.waterData || [],
                                                                                                                    waterGoal: userData.waterGoal || 64,
                                                ...
                                                if (userData.waterGoal) {
                                                        dispatch({ type: 'UPDATE_WATER_GOAL', payload: userData.waterGoal });
                                                }
        ...
        const addWeightEntry = (weight) => {
                dispatch({ type: 'ADD_WEIGHT_ENTRY', payload: weight });
        };

        const updateWeightGoal = (goal) => {
                dispatch({ type: 'UPDATE_WEIGHT_GOAL', payload: goal });
        };

        // Water tracking functions
        const addWaterEntry = (amount) => {
                dispatch({ type: 'ADD_WATER_ENTRY', payload: amount });
        };

        const updateWaterGoal = (goal) => {
                dispatch({ type: 'UPDATE_WATER_GOAL', payload: goal });
        };
